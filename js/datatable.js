...
var DataTable = function () {
    ...
    /**
    *
    * Attach the specified event listener to the specified node.
    *
    * @param node The node to which the event should be attached.
    * @param event The name of the event ("click", "keypress", etc.)
    * @param callback The event listener
    *
    **/
    var addEvent = function (node, event, callback) {
        node.attachEvent ("on" + event, function () { callback.apply (node) ; });
    }
    ...
    /**
    *
    * Add / Check / Remove the specified class(es) to the specified DOM Element.
    *
    ...
    **/
    ...
    function hasClass (node, classes) {
        return node.className.indexOf(classes) > 0;
        /*
        || node.className.indexOf(classes + ' ') == 0
        || node.className.indexOf(' ' + classes) == node.className.length - 1 - classes.length ; */
    }

    function removeClass (node, classes) {
        node.className = node.className.replace(classes, '');
    }
    ...
    **/
    ...
    var DataTable = function (table, opts) {
        ...
        for (var i = 0; i < this.pagingDivs.length; ++i) {
            ...
            addClass(div, 'pagination-datatables');
            addClass(div, this.options.pagingDivClass);
            var ul = document.createElement('ul');
            addClass(ul, this.options.pagingListClass);
            div.appendChild(ul);
        }
    ...
    DataTable.prototype = {
                            ...
                            case 200:
                                if (recursive) {
                                    if (this.response.length > 0) {
    									...
    									for (var i = 0 ; i < syncData.toDelete.length ; ++i) {
    										var e = syncData.toDelete[i] ;
        ...
        updatePaging: function () {
            ...
            for (var i = 0; i < this.pagingLists.length; ++i) {
                ...
                if (dataTable.options.nextPage) {
                    var li = document.createElement('li');
                    if (cp === lp || lp === 0) { addClass(li, 'active'); }
    			...
    			for (var j = 0 ; j < childs.length ; ++j) {
                    ...
                    if (e.childNodes.length > 0) {
                        addEvent(e.childNodes[0], 'click', function () {
        ...
        **/
        destroyFilter: function () {
            removeNode(this.table.querySelector('.datatable-filter-line'));
        },
        ...
        **/
        createTextFilter: function (field) {
            ...
            addClass(input, 'datatable-filter datatable-input-text');
            ...
            addClass(input, this.options.filterInputClass);
            return input;
        },
        ...
        **/
        createSelectFilter: function (field) {
            ...
            addClass(select, 'datatable-filter datatable-select');
            ...
            if (opt instanceof Object && opt.fn instanceof Function) {
                ...
                select.setAttribute('data-filterType', 'function');
            }
            else {
                ...
                select.setAttribute('data-filterType', 'default');
            }
            addClass(select, this.options.filterSelectClass);
            return select;
        },
        ...
        **/
        createFilter: function () {
            ...
            if (this.options.filters) {
                ...
                addClass(tr, 'datatable-filter-line');
                    ...
                    if (this.options.filters.hasOwnProperty(field)) {
                        ...
                        if (this.options.filters[field] !== false) {
                            ...
                            if (!document.body.contains(filter)) {
                                addClass(td, 'datatable-filter-cell');
        ...
        **/
        resetFilters: function () {
            var dtable = this;
    		for (var fi = 0 ; fi < this.filterTags.length ; ++fi) {
    			e = this.filterTags[fi];
            ...
            if (!(this.options.sort instanceof Function)) {
                ...
                for (var i = 0; i < ths.length; ++i) {
                    ...
                    if (ths[i].getAttribute('data-sort')) {
                        ...
                        var key;
                        if (this.options.sort instanceof Array) {
                            key = countTH;
                        }
                        else if (this.options.sort instanceof Object) {
                            key = this._keys(this.options.sort)[countTH];
                        }
                        if (key !== undefined && this.options.sort[key]) {
                            ths[i].setAttribute('data-sort', key);
                        }
                    }

                    if (ths[i].getAttribute('data-sort') !== undefined) {
                        addClass(ths[i], 'sorting');
                    }

                    countTH++;

                    var dataTable = this ;
                    addEvent(ths[i], 'click', function () {
                        if (this.getAttribute('data-sort')) {
                            if (hasClass(this, 'sorting-asc')) {
                                dataTable.options.sortDir = 'desc';
                                removeClass(this, 'sorting-asc')
                                addClass(this, 'sorting-desc');
                            }
                            else if (hasClass(this, 'sorting-desc')) {
                                dataTable.options.sortDir = 'asc';
                                removeClass(this, 'sorting-desc');
                                addClass(this, 'sorting-asc');
                            }
                            else {
                                var oths = this.parentNode.cells;
                                for (var j = 0; j < oths.length; j++) {
                                    removeClass(oths[j], 'sorting-desc');
                                    removeClass(oths[j], 'sorting-asc');
                                }
                                dataTable.options.sortDir = 'asc';
        ...
        triggerSort: function () {
            if (this.options.sort instanceof Function) {
                ...
                for (var j = 0; j < ths.length; j++) {
                    removeClass(ths[j], 'sorting-desc');
                    removeClass(ths[j], 'sorting-asc');
                    if (ths[j].getAttribute('data-sort') === this.options.sortKey) {
                        th = ths[j];
                    }
                }
                if (th !== undefined) {
                    addClass(th, 'sorting-' + this.options.sortDir);
                    this.sort();
                    this.refresh();
                }
            }
        ...
        **/
        refresh: function () {
            ...
            this.updatePaging();
            this.updateCounter();
            removeNode(this.table.tBodies[0]);
        ...
        **/
        destroy: function () {
            ...
            this.destroySort();
            for (var i = 0; i < this.pagingDivs.length; ++i) {
                removeClass(this.pagingDivs[i], 'pagination-datatable');
                removeClass(this.pagingDivs[i], this.options.pagingDivClass);
                this.pagingDivs[i].innerHTML = '';
            }
            this.destroyFilter();
            removeClass(this.table, this.options.tableClass);
            removeNode(this.table.tBodies[0]);
            this.table.appendChild(document.createElement('tbody'));
            for (var i = 0; i < this.data.length; i++) {
                this.table.tBodies[0].appendChild(this.options.lineFormat.call(this.table, this.filterIndex[this.currentStart + i], this.data[this.filterIndex[this.currentStart + i]]));
